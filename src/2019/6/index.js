function Node(name) {
  ...
  this.flatten = () => {
    return this.children.reduce(
      (curr, child) => [...curr, child, ...child.flatten()],
      []
    );
  };
  this.countOrbits = () => {
    return (
      this.flatten().length +
      this.children.reduce((curr, child) => curr + child.countOrbits(), 0)
    );
  };
}
...
export const partTwo = input => {
  ...
  const visitedDict = {};
  let paths = [tree.nodeDict["YOU"].parent];
  let i = 0;
  while (paths.length) {
    paths.forEach(({ name }) => {
      visitedDict[name] = true;
    });
    let newPaths = [];
    paths.forEach(path => {
      newPaths = [
        ...newPaths,
        ...[path.parent, ...path.children].filter(
          node => node && !visitedDict[node.name]
        )
      ];
    });
