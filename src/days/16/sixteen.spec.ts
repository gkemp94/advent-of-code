import { partOne, partTwo, processInput } from "./sixteen";
import { getFile } from "../../utils/getInput";

const SAMPLE_INPUT = processInput(getFile(__dirname, 'sample_1.txt'));
const PUZZLE_INPUT = processInput(getFile(__dirname, 'input.txt'));


describe('Part One', () => {
  it('returns 71 for sample input.', () => {
   expect(partOne(SAMPLE_INPUT, 2020)).toBe(71);
  });

  it('returns correct result for puzzle input', () => {
    expect(partOne(PUZZLE_INPUT, 2020)).toBe(29878);
  });

});

describe('Part Two', () => {
  it('returns 175594 for sample input.', () => {
    expect(partTwo(SAMPLE_INPUT)).toBe(1);
  });

  it('returns correct result for puzzle input', () => {
    const result = partTwo(PUZZLE_INPUT);
    expect(result).toBe(855438643439);
  });
});
